...
public:
	...
	explicit Socket(InternetProtocol internetProtocol)
		: endpoint(socket(getAddressFamily(internetProtocol),
	...
	explicit Socket(Type s) noexcept : endpoint(s) {}
	...
	int connect(const struct sockaddr *address,
		    socklen_t addressSize) noexcept
	{
	...
	int send(const void *buffer, size_t length, int flags) noexcept
	{
		return static_cast<int>(
			::send(endpoint, reinterpret_cast<const char *>(buffer),
			       length, flags));
	}
...
public:
		      ...
		      const std::vector<std::string> &headers)
	{
		...
		addrinfo *info;
		if (getaddrinfo(domain.c_str(), port.c_str(), &hints, &info) !=
		    0)
		...
		std::vector<uint8_t> requestData(headerData.begin(),
						 headerData.end());
		...
		if (socket.connect(addressInfo->ai_addr,
				   static_cast<socklen_t>(
					   addressInfo->ai_addrlen)) == -1)
			throw std::system_error(
				getLastError(), std::system_category(),
				"Failed to connect to " + domain + ":" + port);
		...
		while (remaining > 0) {
			const auto size =
				socket.send(sendData, remaining, noSignal);

			if (size == -1)
				throw std::system_error(
					getLastError(), std::system_category(),
					"Failed to send data to " + domain +
						":" + port);
		...
		for (;;) {
			const auto size = socket.recv(
				tempBuffer, sizeof(tempBuffer), noSignal);

			if (size == -1)
				throw std::system_error(
					getLastError(), std::system_category(),
					"Failed to read data from " + domain +
						":" + port);
			else if (size == 0)
				break; // disconnected

			responseData.insert(responseData.end(), tempBuffer,
					    tempBuffer + size);
			...
			if (parsedHeaders) {
				...
				if (chunkedResponse) {
					...
					break; // Not receiving data on API calls.
